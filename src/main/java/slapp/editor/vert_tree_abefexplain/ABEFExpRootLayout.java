...
import slapp.editor.vert_tree_abefexplain.*;
...
public class ABEFExpRootLayout extends AnchorPane {
    ...
    public ABEFExpRootLayout(VerticalTreeABEFExpView verticalTreeView) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpTreeFormulaBox) {
                            ((ABEFExpTreeFormulaBox) node).processBoxRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
        ...
        starClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpTreeFormulaBox) {
                            ((ABEFExpTreeFormulaBox) node).processStarRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        annotationClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpTreeFormulaBox) {
                            ((ABEFExpTreeFormulaBox) node).processAnnotationRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
  //              event.consume();
        ...
        annotationPlus.setOnAction(e -> {
            ...
            for (Node node : nodesList) {
                if (node instanceof ABEFExpTreeFormulaBox) {
                    ((ABEFExpTreeFormulaBox) node).processAnnotationRequest(true);
                }
            }
        ...
        annotationMinus.setOnAction(e -> {
           ...
           for (Node node : nodesList) {
               if (node instanceof ABEFExpTreeFormulaBox) {
                   ((ABEFExpTreeFormulaBox) node).processAnnotationRequest(false);
               }
           }
            annotationToggle.setSelected(false);
        });
        ...
        circleClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpTreeFormulaBox) {
                            ((ABEFExpTreeFormulaBox) node).processCircleRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        circleToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, circleClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABEFExpTreeFormulaBox) {
                        ((ABEFExpTreeFormulaBox) node).undoCircleRequest();
                    }
                }
            }
        });
        ...
        underlineClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpTreeFormulaBox) {
                            ((ABEFExpTreeFormulaBox) node).processUnderlineRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
//               event.consume();
        ...
        underlineToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, underlineClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABEFExpTreeFormulaBox) {
                        ((ABEFExpTreeFormulaBox) node).undoUnderlineRequest();
                    }
                }
            }
        });
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABEFExpMapFormulaBox) {
                            ((ABEFExpMapFormulaBox) node).processMappingRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        mappingKeyFilter = new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent e) {
                ...
                if (code == KeyCode.F11) {
                    ObservableList<Node> nodesList = main_pane.getChildren();
                    List<ABEFExpMapFormulaBox> formulaBoxes = new ArrayList<>();
                    for (Node node : nodesList) {
                        if (node instanceof ABEFExpMapFormulaBox) {
                    ...
                    if (formulaBoxes.size() == 2) {
                        ABEFExpClickableMapLink mapLink = new ABEFExpClickableMapLink(verticalTreeView);
                        ...
                        if (inHierarchy(verticalTreeView.getMainView().getMainScene().focusOwnerProperty().get(), node)) {
                            if (node instanceof ABEFExpMapFormulaBox) {
                                ...
                                if (mapFormulaBox.getMapStage() > 0) {
                                    ...
                                    ABEFExpMapQuestionMarker mapQuestionMarker = new ABEFExpMapQuestionMarker(verticalTreeView);
        ...
        mappingToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv) {
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABEFExpMapFormulaBox) {
                        ((ABEFExpMapFormulaBox) node).undoMappingRequest();
                    }
                }
            }
        });
 ...
 //               mDragOverIcon.startDragAndDrop (TransferMode.ANY).setContent(content);
    ...
    private void buildDragHandlers() {
                ...
                if (container != null) {
                    if (container.getValue("scene_coords") != null) {
                        ...
                        if (container.getValue("type").equals(DragIconType.dashed_line.toString())) {
                            ABEFExpDashedLine line = new ABEFExpDashedLine(verticalTreeView);
                            ...
                            ABEFExpVerticalBracket bracket = new ABEFExpVerticalBracket(verticalTreeView);
                            ...
                            ABEFExpTreeFormulaBox treeFormulaBox = new ABEFExpTreeFormulaBox(verticalTreeView);
                            ...
                            ABEFExpMapFormulaBox mapFormulaBox = new ABEFExpMapFormulaBox(verticalTreeView);
     ...
     //                  NodeLink link = new NodeLink();
                        ABEFExpClickableNodeLink link = new ABEFExpClickableNodeLink(verticalTreeView);
                        ...
                        for (Node n: main_pane.getChildren()) {
                            ...
                            if (n.getId().equals(sourceId))
                                source = (ABEFExpTreeFormulaBox) n;

                            if (n.getId().equals(targetId))
                                target = (ABEFExpTreeFormulaBox) n;
