...
public class ABExpRootLayout extends AnchorPane {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpTreeFormulaBox) {
                            ((ABExpTreeFormulaBox) node).processBoxRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
        ...
        starClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpTreeFormulaBox) {
                            ((ABExpTreeFormulaBox) node).processStarRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        annotationClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpTreeFormulaBox) {
                            ((ABExpTreeFormulaBox) node).processAnnotationRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
  //              event.consume();
        ...
        annotationPlus.setOnAction(e -> {
            ...
            for (Node node : nodesList) {
                if (node instanceof ABExpTreeFormulaBox) {
                    ((ABExpTreeFormulaBox) node).processAnnotationRequest(true);
                }
            }
        ...
        annotationMinus.setOnAction(e -> {
           ...
           for (Node node : nodesList) {
               if (node instanceof ABExpTreeFormulaBox) {
                   ((ABExpTreeFormulaBox) node).processAnnotationRequest(false);
               }
           }
            annotationToggle.setSelected(false);
        });
        ...
        circleClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpTreeFormulaBox) {
                            ((ABExpTreeFormulaBox) node).processCircleRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        circleToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, circleClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABExpTreeFormulaBox) {
                        ((ABExpTreeFormulaBox) node).undoCircleRequest();
                    }
                }
            }
        });
        ...
        underlineClickFilter = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpTreeFormulaBox) {
                            ((ABExpTreeFormulaBox) node).processUnderlineRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
//               event.consume();
        ...
        underlineToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv)  main_pane.addEventFilter(MouseEvent.MOUSE_PRESSED, underlineClickFilter );
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABExpTreeFormulaBox) {
                        ((ABExpTreeFormulaBox) node).undoUnderlineRequest();
                    }
                }
            }
        });
                ...
                for (Node node : nodesList) {
                    if (inHierarchy(event.getPickResult().getIntersectedNode(), node)) {
                        if (node instanceof ABExpMapFormulaBox) {
                            ((ABExpMapFormulaBox) node).processMappingRequest(event.getButton() == MouseButton.PRIMARY);
                            break;
                        }
                    }
                }
 //               event.consume();
        ...
        mappingKeyFilter = new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent e) {
                ...
                if (code == KeyCode.F11) {
                    ObservableList<Node> nodesList = main_pane.getChildren();
                    List<ABExpMapFormulaBox> formulaBoxes = new ArrayList<>();
                    for (Node node : nodesList) {
                        if (node instanceof ABExpMapFormulaBox) {
                    ...
                    if (formulaBoxes.size() == 2) {
                        ABExpClickableMapLink mapLink = new ABExpClickableMapLink(verticalTreeView);
                        ...
                        if (inHierarchy(verticalTreeView.getMainView().getMainScene().focusOwnerProperty().get(), node)) {
                            if (node instanceof ABExpMapFormulaBox) {
                                ...
                                if (mapFormulaBox.getMapStage() > 0) {
                                    ...
                                    ABExpMapQuestionMarker mapQuestionMarker = new ABExpMapQuestionMarker(verticalTreeView);
        ...
        mappingToggle.selectedProperty().addListener((ob, ov, nv) -> {
            if (nv) {
                ...
                for (Node node : nodesList) {
                    if (node instanceof ABExpMapFormulaBox) {
                        ((ABExpMapFormulaBox) node).undoMappingRequest();
                    }
                }
            }
        });
...
//                mDragOverIcon.startDragAndDrop (TransferMode.ANY).setContent(content);
    ...
    private void buildDragHandlers() {
     ...
     //                  NodeLink link = new NodeLink();
                        ABExpClickableNodeLink link = new ABExpClickableNodeLink(verticalTreeView);
                        ...
                        for (Node n: main_pane.getChildren()) {
                            ...
                            if (n.getId().equals(sourceId))
                                source = (ABExpTreeFormulaBox) n;

                            if (n.getId().equals(targetId))
                                target = (ABExpTreeFormulaBox) n;
